...
test("默认库完整性（M-R3）：写作/剧情指导常驻、六拉取包就位，frontmatter 合格、无乱码", () => {
	const repo = join(import.meta.dirname, "..");
	const files = scanSkillFiles(repo);
	const byName = new Map(files.map((f) => [f.name, f]));
	assert.equal(byName.get("写作")?.resident, true, "写作 常驻");
	// 剧情指导：常驻（8/11 用户定案：读过就在场，不逐段重读——受理门与每段读取指令随 resident 翻转自动解除）
	const guide = byName.get("剧情指导");
	assert.ok(guide && guide.resident && guide.body.length > 0, "剧情指导 在场且常驻");
